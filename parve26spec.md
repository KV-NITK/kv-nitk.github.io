...
- **Scene 6 posters** are drawn in code as animated SVG, one motif per kind of event. Only singer and guest posters get a real photo, through the `photo` field in `content/events.js`.
...
| 0 Lamp, 1 Curtain and countdown | Paused | Pieces built but not wired in, in `scenes/00-intro/`: `lamp.jsx`, `smoke.js` (smoke letters), `leader.js` (೫ ೪ ೩ ೨ countdown), `stage-curtain.js` (curtain that shows the projection bent over its folds, opens and closes). Still to do: the darkness overlay, the sequence on the page (camera close on the lamp, lights coming up, curtain, then title), scroll lock, Skip tab, returning-visitor skip, reduced motion. |
...
| 5 Fan pass | Paused | Only `scenes/05-fan-pass/transliterate.js` (rule-based English→Kannada spellings) exists. A placeholder section is on the page. |
...
| 11 Which film? | Built | The projection booth: the projector side-on (its footage counter is the timer; the reels turn while it runs), the port window onto the hall, a bare bulb, the workbench, a reel shelf and the operators' duty chart. Five cans a day, the same for everyone (a window into the puzzle pool that moves on at midnight IST). Each can: three Twemoji stickers (alt text describes the pictures), masking tape to write the name on in English or Kannada (typos forgiven), three tries punched as holes, a hint tag (year stamped on the rim, then the first letter; −5 each). Right: a tick stamp and the title flickers on the distant screen; out of tries: the answer in red pencil and a red tape cross; the can rolls off to the shelf. Score: 60 plus up to 40 for speed, less for wrong tries and hints. After five: the day's log with Share (the phone share sheet, or copy), a name for the chart, and "New reels in …"; no replays (kept in the browser). Duty chart: stand-in top 10 with your row highlighted. On a phone the note moves below the can and the chart opens from a button. |
| 12 Credits and ಶುಭಂ | Built | A silent black beat, then "ಕನ್ನಡ ವೇದಿಕೆ ಅರ್ಪಿಸುವ · ಪರ್ವ 2026" and the end credits in projected cream: the core team from the home page's team list (`src/data/teamData.js`, plus the members whose entries there wait for photos), each team role credited as a film job (Convenor → Directed by, Media → Cinematography, Technical → Special effects, Literary → Dialogue and lyrics, …), the faculty advisor under Guidance, special thanks to the sponsors, the thank-you card, what we used (fonts, Twemoji, photos) and contact, social links and (once they exist) policy links. Beside the roll, a making-of inset of the club's photos, sepia developing into colour, with a link to the gallery (Parva 2025's for now); it stays in place on a laptop. When you stop scrolling for 3 s the roll drifts up by itself, pausing on the thank-you card and on the stinger "ಸಮುದ್ರಪ್ಪ ಡೂಮ್ಸ್‌ಡೇಯಲ್ಲಿ ಮತ್ತೆ ಬರುತ್ತಾರೆ? · Samudrappa will return in Doomsday?". Then ಶುಭಂ and ಸಿರಿಗನ್ನಡಂ ಗೆಲ್ಗೆ, the curtain closes over the screen, and the stage front offers "ಮತ್ತೊಮ್ಮೆ ನೋಡಿ · Watch again" and one last Book. |

### Where the code lives
Route `/parva-26`, lazy-loaded from `src/App.jsx`. `/parva` still shows Parva 2025 until launch. Everything is in `src/components/Parva26/`, with a one-screen map in its `README.md`. Imports use two aliases: `@p26/…` for this folder and `@/…` for `src` (both in `vite.config.js` and `jsconfig.json`).
- **`Parva26.jsx`:** the page: fonts, the always-on pieces, and the scenes in show order (which ones sit in a `FilmFrame`).
- **`scenes/NN-name/`:** one folder per scene, numbered as in this spec. `index.jsx` is the scene; everything only that scene uses sits beside it. `00-intro` (lamp, smoke letters, countdown leader, stage curtain) and `05-fan-pass` (the stub and the English→Kannada spelling helper) are parked. Of note: `02-title/index.jsx` has the camera push (`useCameraPush`), `forest.js` and `air.js` are the hero's canvases, `04-gandhada-gudi/sandal-strip.js` paints the whole film strip, `07-guests/guest-frame.jsx` is one portrait (lamp, veil, tag, labels and the unveiling in their own files), `08-interval/` has the interval move (`index.jsx`), the feast poster, the booking counter and the tee showcase (`tee-3d.jsx`, `showcase-parts.jsx`), `09-release-day/samudrappa.jsx` is the cutout and `petals.js` the flower canvas, `10-timeline/era-cards.jsx` the title cards, `11-emoji-game/answers.js` the answer check, `12-credits/` the roll, making-of inset, drift and ending.
- **`chrome/`:** on every screen: `top-bar.jsx`, `film-reel.jsx`, `subtitle-strip.jsx` (`sayLine()` for scene lines, `anchorStrip()` to sit on the screen), `agarbatti-cursor.jsx`.
- **`film/`:** the on-screen look: `film-layer.jsx` (grain and flicker canvas; `useFilmScreen`, `useFilmTick`), `film-frame.jsx` (the frame and cue mark), `ink-defs.jsx` (the rubber-stamp SVG filter).
- **`ui/`:** theatre pieces used by more than one scene: `coupon.jsx` (the Book coupon), `flourish.jsx`, `bulbs.jsx` (marquee bulbs), `lettering.js` (the ಪರ್ವ extrusion). A scene never imports from another scene; anything shared moves here.
- **`lib/`:** `gsap.js` (GSAP with ScrollTrigger and DrawSVG registered), `prefs.jsx` (subtitles and sound; `Sub` and `En`, the English subtitle that stays for screen readers when subtitles are off), `on-screen.js` (`useOnScreen`, `PAUSED`), `storage.js` (guarded `load`/`save`, `useStoredState`), `layers.js` (`willChange()`), `text.js` (`graphemes`, Kannada digits), `use-current-scene.js`, `use-reduced-motion.js`.
- **`styles/`:** `materials.js`, `textures.js` and `carving.js`: finishes as style objects.
- **`content/`:** everything the team will change, one file per topic (event, events, guests, sponsors, interval, hoomale, timeline, puzzles, credits, scenes), re-exported from `content/index.js`; import from `@p26/content`. Stand-ins are marked `TODO(content)`.
- **`assets/`:** `textures/` (baked WebP tiles: paper, wood, sandal panel, door quilt, iron gate), `guests/` (past guests' portraits), `emoji/` (Twemoji SVGs, CC BY 4.0, credited in Scene 12), `making-of/` (small copies of the gallery photos). Sponsor logos are still read from `public/sponsors/`.
- **Checks:** `scripts/parva26-check.mjs` screenshots every scene (reduced motion, laptop and phone, fixed clock and random seed) and compares with a saved baseline; `P26_SUBTITLES=off` does the same with subtitles off. `scripts/parva26-flows.mjs` plays the main interactions. Both run against `npx vite build && npx vite preview --port 5198`.
...
1. **One canvas per scene for anything that moves all the time,** on the shared 12 fps tick (`useFilmTick`), paused when off screen (`useOnScreen` from `lib/on-screen.js`). Paint still parts once into offscreen canvases and only composite them each frame (see `forest.js`, `sandal-strip.js`). Cap each canvas's pixel count (the hero forest uses about 480k pixels; the air canvas runs at half resolution).
2. **Never scale or fade a big DOM subtree without promoting it.** Scaling the hall repainted everything at every zoom step: about 5.4 s of painting in a 3 s scroll. Give the moving parts `will-change` only while they move and remove it about 200 ms after they stop, so they repaint once, sharp. The pattern is in `useCameraPush` in `scenes/02-title/index.jsx`; set the hint with `willChange()` from `lib/layers.js`. The interval walk-out (Scene 8) and the ending pull-back (Scene 12) must reuse it. The exception is something that only slides and never scales (the Scene 7 wall): keep its layer for as long as it is near the screen, or every new scroll re-rasters it.
...
### What is left
- **Scenes:** resume the opening (0–1) and the fan pass (5).
- **Scene 12 not built:** the house lights and pull-back over the whole theatre at the end (the audience leaving, the diya still burning), the music and applause, single-colour sponsor logos (names only for now), the policy pages, Kannada spellings of the team's names.
...
- **Scene 11 not built:** answer checking and the leaderboard on the server (the 10 stand-in puzzles and their answers are in the page for now, readable by anyone), the name filter, sounds (hum, slide, punch, clunk, cheer), dust in the vent light. The duty chart is typed (Special Elite) rather than hand-written.
