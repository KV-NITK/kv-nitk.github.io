...
- **Other built scenes:** `scene-certificate.jsx`, `scene-gandhada-gudi.jsx`, `scene-now-showing.jsx`, `poster-art.jsx`, `scene-guests.jsx` (the lobby wall, pan, producers' board) and `honour-frame.jsx` (one frame: lamp, veil, tag, nameplate, unveiling), `scene-interval.jsx` (the interval move and the lobby), `feast-poster.jsx` (banana leaf and dishes), `booking-counter.jsx` (the booking window and coupon), `tee-showcase.jsx` (showcase, 3D tee, choices, Buy now).
- **Images:** `assets/guests/` (past guests' portraits as small WebP), `assets/sandal-panel.webp` (the lobby wall's grain tile), `assets/door-quilt.webp` (the swing doors' quilted rexine). Sponsor logos are still read from `public/sponsors/`.
...
### What is left
- **Scenes:** 9 release-day cutout; 10 rewind bench; 11 projection booth game; 12 credits and ಶುಭಂ; then resume the opening (0–1) and the fan pass (5).
...
- **Scene 8 extras not built:** the phone's sticky Book button while the food station is in view, the glass door creaking open, sounds.
